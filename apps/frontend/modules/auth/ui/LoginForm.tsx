...
export function AuthForm({ mode }: AuthFormProps) {
    ...
    const switchText = mode === 'login' ? 'Нет аккаунта? Регистрация' : 'Уже есть аккаунт? Войти'

    return (
        <Box component='form' onSubmit={onSubmit} sx={{ maxWidth: 380, mx: 'auto', mt: 8, display: 'grid', gap: 2 }}>
            ...
            <Button type='button' variant='text' onClick={() => router.push(switchHref)} sx={{ justifySelf: 'start', px: 0 }}>
                {switchText}
            </Button>
        </Box>
    )
