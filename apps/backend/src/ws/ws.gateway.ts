...
export class WsGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    async joinChat(@MessageBody() dto: ChatJoinDto, @ConnectedSocket() client: Socket) {
    ...
    async newMessage(@MessageBody() dto: SendMessageDto, @ConnectedSocket() client: Socket) {
        this.ensureAuth(client)

        const user = client.data.user as { id: string; login?: string }

        // TODO: позже тут будет Prisma create + нормальная модель
        const message = {
